...
class ModelRepository:
		...
		try:
			with get_connection() as conn:
				...
				conn.execute(
					...
					""",
					(
						data["name"], data["model_id"], data["model_type"], data["base_url"], data["api_key"],
						data["temperature"], data["max_tokens"], data.get("system_prompt", ""),
						data["support_stream"], data["support_think"], data["is_default"], data["status"]
					)
				)
			return True, None
	...
	def update_model(model_id: int, data: dict):
		with get_connection() as conn:
			row = conn.execute("select id from ai_models where id=?", (model_id,)).fetchone()
			if not row:
				return False, "模型不存在"
			if data.get("is_default"):
	...
	def usage_summary(model_id: int = None):
		with get_connection() as conn:
			if model_id:
				...
				return dict(prompt=row["prompt"], completion=row["completion"], total=row["total"], calls=row["calls"])
			rows = conn.execute(
				"""
