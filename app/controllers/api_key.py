...
class AdminApiKeyHandler(AdminBaseHandler):
    def get(self):
        ...
        with get_connection() as conn:
            total = conn.execute(f"SELECT COUNT(*) FROM api_keys {where}", params).fetchone()[0]
        ...
        if edit_id.isdigit():
            with get_connection() as conn:
                edit_api = conn.execute("SELECT * FROM api_keys WHERE id=?", (int(edit_id),)).fetchone()
        self.render("admin/apis.html", title="接口管理", username=self.current_user,
                    api_keys=api_keys, total=total, page=page, per_page=PER_PAGE,
                    keyword=keyword, edit_api=edit_api, msg=self._message())
