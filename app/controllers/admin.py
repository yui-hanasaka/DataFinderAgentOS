...
from app.models.admin import AdminRepository, PER_PAGE
...
class AdminRoleHandler(AdminBaseHandler):
	def get(self):
		...
		edit_role = AdminRepository.get_role(int(edit_id)) if edit_id.isdigit() else None
		checked_menu_ids = AdminRepository.get_role_menu_ids(int(edit_id)) if edit_id.isdigit() else []
		self.render(
			"admin/roles.html",
			title="角色管理",
	...
	def post(self):
		...
		if action == "delete" and role_id.isdigit():
			...
			return self._redirect_with_message("/admin/roles", msg or "角色已删除" if ok else msg)
		...
		if role_id.isdigit():
			...
			return self._redirect_with_message("/admin/roles", msg or "角色已更新" if ok else msg)
...
class AdminUserHandler(AdminBaseHandler):
	def get(self):
		...
		edit_id = self.get_query_argument("edit", "")
		edit_user = AdminRepository.get_user(int(edit_id)) if edit_id.isdigit() else None
		self.render(
			"admin/users.html",
			title="用户管理",
			username=self.current_user,
	...
	def post(self):
		...
		if action == "delete" and user_id.isdigit():
			...
			return self._redirect_with_message("/admin/users", msg or "用户已删除" if ok else msg)
		...
		if user_id.isdigit():
			...
			return self._redirect_with_message("/admin/users", msg or "用户已更新" if ok else msg)
...
class AdminMenuHandler(AdminBaseHandler):
	def get(self):
		...
		edit_id = self.get_query_argument("edit", "")
		edit_menu = AdminRepository.get_menu(int(edit_id)) if edit_id.isdigit() else None
		self.render(
			"admin/menus.html",
			title="功能管理",
			username=self.current_user,
	...
	def post(self):
		...
		if action == "delete" and menu_id.isdigit():
			...
			return self._redirect_with_message("/admin/menus", msg or "功能已删除" if ok else msg)
		...
		if menu_id.isdigit():
			...
			return self._redirect_with_message("/admin/menus", msg or "功能已更新" if ok else msg)
