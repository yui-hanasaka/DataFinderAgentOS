...
class AdminEmployeeHandler(AdminBaseHandler):
    def get(self):
        ...
        edit_emp = EmployeeRepository.get_employee(int(edit_id)) if edit_id.isdigit() else None
        ...
        self.render("admin/employees.html", title="数字员工", username=self.current_user,
                    employees=employees, total=total, page=page, per_page=PER_PAGE,
                    keyword=keyword, edit_employee=edit_emp, all_models=all_models,
    ...
    def post(self):
        ...
        if action == "delete" and emp_id.isdigit():
            ...
            return self._redirect_with_message("/admin/employees", msg or "已删除" if ok else msg)
        ...
        if emp_id.isdigit():
            ok, msg = EmployeeRepository.update_employee(int(emp_id), name, avatar, model_id, system_prompt, skills_list, status)
            return self._redirect_with_message("/admin/employees", msg or "已更新" if ok else msg)
        ok, msg = EmployeeRepository.create_employee(name, avatar, model_id, system_prompt, skills_list, status)
