...
class AdminPermissionHandler(AdminBaseHandler):
    def get(self):
        ...
        role_menu_map = {r["id"]: set(AdminRepository.get_role_menu_ids(r["id"])) for r in roles}
        self.render("admin/permissions.html", title="权限管理", username=self.current_user,
                    roles=roles, menus=menus, role_menu_map=role_menu_map, msg=self._message())
