...
class AdminModelEngineHandler(AdminBaseHandler):
	...
	def post(self):
		...
		if action == "delete" and model_id.isdigit():
			...
			return self._redirect_with_message("/admin/models", msg or "模型已删除" if ok else msg)
		if action == "set_default" and model_id.isdigit():
			ok, msg = ModelRepository.set_default(int(model_id))
			return self._redirect_with_message("/admin/models", msg or "已设为默认模型" if ok else msg)
		...
		data = {
			...
			"support_stream": 1 if self.get_body_argument("support_stream", "0") == "1" else 0,
			"support_think": 1 if self.get_body_argument("support_think", "0") == "1" else 0,
		...
		if model_id.isdigit():
			...
			return self._redirect_with_message("/admin/models", msg or "模型已更新" if ok else msg)
...
class AdminModelChatHandler(AdminBaseHandler):
	...
	async def post(self, model_id):
		...
		if think:
			messages.append({"role": "system", "content": "请按 <think>...</think> 输出推理过程后再给出答案。"})
	...
	async def _sync_response(self, model, messages):
		resp = chat_complete(
			model["base_url"], model["api_key"], model["model_id"], messages,
			temperature=model["temperature"], max_tokens=model["max_tokens"], stream=False
		)
		raw = resp.read()
		parsed = parse_chat_response(raw)
		ModelRepository.record_usage(model["id"], parsed["prompt_tokens"], parsed["completion_tokens"])
	...
	async def _stream_response(self, model, messages):
		...
		resp = chat_complete(
			model["base_url"], model["api_key"], model["model_id"], messages,
			temperature=model["temperature"], max_tokens=model["max_tokens"], stream=True
		)
		prompt_tokens = 0
		completion_tokens = 0
		for chunk in iter_sse_chunks(resp):
			...
			self.write("data: " + json.dumps({"content": text, "reasoning": reasoning}) + "\n\n")
			await self.flush()
