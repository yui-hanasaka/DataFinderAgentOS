...
class AdminSkillHandler(AdminBaseHandler):
    def get(self):
        ...
        edit_skill = SkillRepository.get_skill(int(edit_id)) if edit_id.isdigit() else None
        self.render("admin/skills.html", title="技能管理", username=self.current_user,
                    skills=skills, total=total, page=page, per_page=PER_PAGE,
                    keyword=keyword, edit_skill=edit_skill, msg=self._message())
    ...
    def post(self):
        ...
        if action == "delete" and skill_id.isdigit():
            ...
            return self._redirect_with_message("/admin/skills", msg or "已删除" if ok else msg)
        ...
        if skill_id.isdigit():
            ok, msg = SkillRepository.update_skill(int(skill_id), name, skill_type, config_json, status)
            return self._redirect_with_message("/admin/skills", msg or "已更新" if ok else msg)
        ok, msg = SkillRepository.create_skill(code, name, skill_type, config_json, status)
